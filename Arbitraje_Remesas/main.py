...
def debug_solanda(db: Session = Depends(get_db)):
    ...
    error = None
    try:
        # 1. Merge "Solanda" client (any ID) into "Solanda Gomez" (ID 3)
        ...
        if solanda and solanda_gomez:
            log.append(f"Merging client '{solanda.nombre}' (ID: {solanda.id}) into '{solanda_gomez.nombre}' (ID: {solanda_gomez.id})...")
            # Update remesas
        ...
        # 2. Fix Zelle ID 112 (Today's Solanda $50)
        z112 = db.query(MovimientoZelle).filter(MovimientoZelle.id == 112).first()
        if z112:
            log.append(f"Fixing Zelle ID 112: linking to Remesa 116")
            z112.remesa_id = 116
            z112.estado = "remesado"
            z112.cliente_nombre = "Solanda Gomez"
            z112.detalle = "Registrado vía Bot de Telegram [Remesado - Remesa ID #116]"
            db.commit()
            
        # 3. Fix Zelle ID 113 (Today's Hermagora $50)
        z113 = db.query(MovimientoZelle).filter(MovimientoZelle.id == 113).first()
        if z113:
            log.append(f"Fixing Zelle ID 113: resetting to pendiente (no remesa yet)")
            z113.remesa_id = None
            z113.estado = "pendiente"
            z113.cliente_nombre = "Hermagora"
            z113.detalle = "Registrado vía Bot de Telegram"
            db.commit()
            
        # 4. Clean up any Zelle movements that have client_nombre = 'solanda'
        updated_zelle = db.execute(text("UPDATE movimientos_zelle SET cliente_nombre = 'Solanda Gomez' WHERE lower(cliente_nombre) = 'solanda'")).rowcount
        db.commit()
        log.append(f"Updated {updated_zelle} Zelle movements names to 'Solanda Gomez'.")
    ...
    remesas = db.execute(text("SELECT id, fecha, cliente_nombre, monto_usd, metodo_pago, banco_receptor FROM historial_remesas WHERE id IN (37, 44, 45, 100, 109, 112, 113, 116)")).fetchall()
    zelle = db.execute(text("SELECT id, fecha, cliente_nombre, titular, monto, estado, remesa_id, detalle FROM movimientos_zelle WHERE id IN (112, 113)")).fetchall()
    
    return {
        "log": log,
        "error": error,
        "clients": [{"id": c[0], "nombre": c[1]} for c in clients],
        "remesas": [{"id": r[0], "fecha": str(r[1]), "cliente_nombre": r[2], "monto_usd": r[3], "metodo_pago": r[4], "banco_receptor": r[5]} for r in remesas],
